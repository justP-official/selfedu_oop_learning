...
class PrimaryKeyError(Exception):
    def __init__(self, *args, **kwargs):
        if not kwargs:
            self.error_text = "Первичный ключ должен быть целым неотрицательным числом"
        else:
            if 'id' in kwargs:
                self.error_text = f"Значение первичного ключа id = {kwargs['id']} недопустимо"
            else:
                self.error_text = f"Значение первичного ключа pk = {kwargs['pk']} недопустимо"

    def __str__(self):
        return self.error_text


pke = PrimaryKeyError(pk=2)
