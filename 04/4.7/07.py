...
class Note:
    @staticmethod
    def __check__name(name):
        if type(name) != str or name not in ('до', 'ре', 'ми', 'фа', 'соль', 'ля', 'си'):
            raise ValueError('недопустимое значение аргумента')

    @staticmethod
    def __check__ton(ton):
        if type(ton) != int or not -1 <= ton <= 1:
            raise ValueError('недопустимое значение аргумента')

    def __init__(self, name, ton):
        self.__check__name(name)
        self._name = name

        self.__check__ton(ton)
        self._ton = ton

    def __setattr__(self, key, value):
        if key == '_name':
            self.__check__name(value)
        elif key == '_ton':
            self.__check__ton(value)
        object.__setattr__(self, key, value)


class Notes:
    @classmethod
    def __check_index(cls, indx):
        if type(indx) != int or not 0 <= indx < len(cls.__slots__):
            raise IndexError('недопустимый индекс')
