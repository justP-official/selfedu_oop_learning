...
class Item:
    ...
    def __verify_value(value):
        if not isinstance(value, (int, float, Item)):
            raise ArithmeticError

    def __add__(self, other):
        self.__verify_value(other)
        money = other.money if isinstance(other, Item) else other
        return int(self.money + money)
...
class Budget:
    ...
    def add_item(self, it):
        self.family_budget.append(it)

    def remove_item(self, indx):
        self.family_budget.pop(indx)

    def get_items(self):
        return self.family_budget
